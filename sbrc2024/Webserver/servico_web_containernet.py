...
def topology():
    ...
    d1 = net.addDocker('cliente', ip='10.0.0.249', volumes=['/tmp/.X11-unix:/tmp/.X11-unix:rw'],
                       privileged=True, environment={'DISPLAY': ":0"},
                       dimage="devopsorchestrator/validacao:latest",
                       port_bindings={4622: 22})

    d2 = net.addDocker('postgres', ip='10.0.0.250', dimage="devopsorchestrator/postgres_validacao:latest",
                       port_bindings={5432: 5432}, volumes=["doo-postgres-data:/var/lib/postgresql/data"],
                       environment={"POSTGRES_DB": "doo",
                                    "POSTGRES_PASSWORD": "postgres",
                                    "POSTGRES_USER": "postgres"})
    
    d3 = net.addDocker('gitlab', ip='10.0.0.251', dimage="gitlab/gitlab-ce:latest",
    ...
    d4 = net.addDocker('runner', ip='10.0.0.252', dimage="devopsorchestrator/gitlab_runner:latest",
    ...
    d5 = net.addDocker('doo', ip='10.0.0.253', dimage="devopsorchestrator/doo:latest",
    ...
    d6 = net.addDocker('apache1', ip='10.0.0.254', volumes=['/tmp/.X11-unix:/tmp/.X11-unix:rw'],
                       privileged=True, environment={'DISPLAY': ":0"},
                       dimage="devopsorchestrator/validacao:latest",
    ...
    d1.cmd('service ssh start')
    d2.cmd('/usr/local/bin/docker-entrypoint.sh postgres &')
    d3.cmd('/assets/wrapper &')
    d4.cmd('gitlab-runner start &')
    d5.cmd('/home/app/doo/docker-entrypoint.sh &')
